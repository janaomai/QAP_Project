...
def run():
    ...
    medians = {analyte: round(np.median(Database_cleaned[analyte].dropna()), 2) for analyte in analytes}
    ...
    for index, row in Database.iterrows():
        ...
        for i, analyte in enumerate(analytes):
            ...
            if pd.isna(your_result):
                ...
                row_cells[2].text = f"{limits[analyte]['ll']:.2f}"
                row_cells[3].text = f"{medians[analyte]:.2f}"
                row_cells[4].text = f"{limits[analyte]['ul']:.2f}"
        ...
        for idx, analyte in enumerate(analytes):
            ...
            ax.axhspan(limits[analyte]['ll'], medians[analyte], color='green', alpha=0.2)
            ax.axhspan(medians[analyte], limits[analyte]['ul'], color='green', alpha=0.2)
            if not (limits[analyte]['ll'] <= your_result <= limits[analyte]['ul']):
