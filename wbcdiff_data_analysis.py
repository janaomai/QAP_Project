...
def run():
    ...
    medians = {bloodcell: round(np.median(Database_cleaned[bloodcell].dropna()), 1) for bloodcell in bloodcells}
    medians['wcc'] = round(medians['wcc'], 1)  
    medians_percent = {bloodcell + '_percent': round(np.median(Database_cleaned[bloodcell + '_percent'].dropna()), 1) for bloodcell in bloodcells[1:]}
    ...
    for index, row in Database.iterrows():
        ...
        for i, bloodcell in enumerate(bloodcells):
            ...
            if pd.isna(your_result):
                ...
                if bloodcell == 'wcc':
                    row_cells[1].text = "{:.1f}".format(your_result)
                    row_cells[2].text = "{:.1f}".format(limits[bloodcell][0])
                    row_cells[3].text = "{:.1f}".format(medians[bloodcell])
                    row_cells[4].text = "{:.1f}".format(limits[bloodcell][1])
                    row_cells[5].text = bloodcell_units[bloodcell]  
        ...
        for idx, bloodcell in enumerate(bloodcells):  
            ...
            if bloodcell == 'wcc':
                ...
                if limits[bloodcell][0] <= your_result_wcc <= limits[bloodcell][1]:
                    ax.set_title('Acceptable', fontsize=20, fontweight='bold', color='green')
                else:
                    ax.set_title('Unacceptable', fontsize=20, fontweight='bold', color='red')

                ax.annotate('RCPA ALP: +/- 0.5 up to 5x10⁹/L\nthen 10%', xy=(0, -0.18), xycoords='axes fraction', fontsize=10, ha='left')
                ...
                if bloodcell == 'eosino':
                    ...
                    min_val = Database_cleaned[bloodcell + '_percent'].min()
                    max_val = Database_cleaned[bloodcell + '_percent'].max()
                ...
                ax.annotate(additional_text[bloodcell], xy=(0, -0.18), xycoords='axes fraction', fontsize=10, ha='left')

            ax.legend(loc='upper right', bbox_to_anchor=(1.15, 0), fontsize=15)
            ax.set_xticks([])
